  ...
  switch (categoryKey) {
    ...
    case 'gpu':
      return [
        `${item.vram} GB VRAM`,
        `${item.tdp}W TDP`,
        item.used ? 'Used' : item.legacy ? 'Legacy' : 'New',
      ];
    case 'ram':
    ...
    case 'case':
      return [item.formFactor, item.used ? 'Used' : 'New'];
    default:
      return [];
  }
}
...
function checkCompatibility(selected) {
  ...
  if (selected.ram && selected.cpu) {
    if (selected.ram.type === 'DDR5' && selected.cpu.socket !== 'LGA1700') {
      issues.push('DDR5 RAM needs LGA1700 socket CPU');
    }
    if (selected.ram.type === 'DDR3' && !selected.cpu.legacy) {
      issues.push('DDR3 RAM only fits legacy platforms');
    }
  }
  // PSU headroom check
  if (selected.psu) {
    ...
    if (totalTdp > selected.psu.wattage) {
      issues.push(`PSU too weak (${selected.psu.wattage}W < ~${totalTdp}W needed)`);
    }
  }
  return issues;
}
...
function CompatBadge({ selected, item, categoryKey }) {
  ...
  return (
    <span
      ...
      title={ok ? 'Compatible' : issues.join(', ')}
    >
      {ok ? '\u2713' : '\u2717'}
    </span>
  );
    ...
    <div className="min-h-screen bg-surface pcb-bg text-gray-200 flex flex-col font-mono">
      ...
      <header className="border-b border-border bg-surface-light/80 backdrop-blur px-6 py-4">
        ...
        <p className="text-xs text-gray-500 mt-1 tracking-wide">
          Select components to build your rig
        </p>
      </header>
          ...
          <div className="mt-auto px-5 pt-6 border-t border-border space-y-2">
            ...
            {Object.entries(selectedComponents).map(([key, comp]) => (
              <div key={key} className="flex items-center justify-between text-xs">
                ...
                <span className="text-neon-green font-mono">${comp.price}</span>
              </div>
            ))}
          </div>
        </aside>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {isExpanded && (
                      <div className="pt-2 border-t border-border text-[11px] text-gray-500 space-y-1">
                        ...
                        {item.legacy && (
                          <p className="text-neon-yellow">Legacy component</p>
                        )}
                        {item.used && (
                          <p className="text-neon-yellow">Used / refurbished</p>
                        )}
                      </div>
                    )}
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 border-t border-border bg-surface-light/95 backdrop-blur-lg px-6 py-3 flex items-center gap-6 z-50">
        ...
        <div className="flex items-center gap-2">
          ...
          {bottleneck ? (
            ...
            <span className="text-xs text-gray-600 italic">Pick CPU &amp; GPU</span>
          )}
        </div>
