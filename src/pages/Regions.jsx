...
const TABS = ["Regions", "Compare", "Pricing", "Anti-Fraud"];
...
const RADAR_AXES = ["Import Ease", "Market Size", "Margin", "Competition", "Volume"];

/** Normalise a region's raw data into 0–100 scores for the 5 radar axes. */
function radarScores(key) {
  const r = REGIONS[key];
  return {
    "Import Ease": Math.max(0, 100 - r.importDuty * 2 - r.vat),
    "Market Size": r.marketSize,
    Margin: r.margin * 3,
    Competition: 100 - r.marketSize * 0.5,
    Volume: r.flipVolume,
  };
}
...
const SCENARIOS = [
  ...
  {
    id: "cis-brazil",
    label: "CIS→Brazil cheat",
    verdict: "BLOCKED",
    trust: 18,
    verdictColor: "text-neon-red",
    badgeBg: "bg-neon-red/20 border-neon-red/40",
    signals: [
      { status: "red", note: "Brazil" },
      { status: "red", note: "CIS IP" },
      { status: "red", note: "+7 RU" },
      { status: "yellow", note: "R$" },
      { status: "yellow", note: "pt-BR" },
      { status: "red", note: "RU card" },
    ],
  },
  {
    id: "vpn-fake",
    ...
    badgeBg: "bg-neon-red/20 border-neon-red/40",
    signals: [
      { status: "red", note: "No listings" },
      { status: "red", note: "VPN detected" },
      { status: "red", note: "VoIP number" },
      { status: "red", note: "Crypto" },
      { status: "yellow", note: "en-US" },
      { status: "red", note: "Prepaid" },
    ],
  },
  {
    id: "relocation",
    label: "Legit relocation",
    verdict: "REVIEW",
    trust: 62,
    verdictColor: "text-neon-yellow",
    badgeBg: "bg-neon-yellow/20 border-neon-yellow/40",
    signals: [
      { status: "green", note: "AU listings" },
      { status: "yellow", note: "US IP" },
      { status: "green", note: "+61 AU" },
      { status: "yellow", note: "USD" },
      { status: "green", note: "en-AU" },
      { status: "yellow", note: "US card" },
    ],
  },
];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs mb-3">
        <div className="text-gray-500">
          Import Duty <span className="text-white font-mono">{r.importDuty}%</span>
        </div>
        <div className="text-gray-500">
          VAT <span className="text-white font-mono">{r.vat}%</span>
        </div>
        <div className="text-gray-500">
          Avg Flip <span className="text-neon-green font-mono">{r.currency}{r.avgFlipCheck}</span>
        </div>
        <div className="text-gray-500">
          Margin <span className="text-neon-green font-mono">{r.margin}%</span>
        </div>
        <div className="text-gray-500">
          Flipper <span className="text-white font-mono">{r.currency}{r.subPrice.flipper}/mo</span>
        </div>
        <div className="text-gray-500">
          Pro <span className="text-neon font-mono">{r.currency}{r.subPrice.pro}/mo</span>
        </div>
      </div>
...
function CompareTab({ regionA, setRegionA, regionB, setRegionB }) {
  ...
  const metrics = [
    { label: "Import Duty", a: `${rA.importDuty}%`, b: `${rB.importDuty}%`, delta: rB.importDuty - rA.importDuty, suffix: "pp", lower: true },
    { label: "VAT", a: `${rA.vat}%`, b: `${rB.vat}%`, delta: rB.vat - rA.vat, suffix: "pp", lower: true },
    { label: "Avg Flip Check", a: `${rA.currency}${rA.avgFlipCheck}`, b: `${rB.currency}${rB.avgFlipCheck}`, delta: rB.avgFlipCheck - rA.avgFlipCheck, suffix: "", lower: false },
    { label: "Margin", a: `${rA.margin}%`, b: `${rB.margin}%`, delta: rB.margin - rA.margin, suffix: "pp", lower: false },
    { label: "Market Size", a: rA.marketSize, b: rB.marketSize, delta: rB.marketSize - rA.marketSize, suffix: "", lower: false },
    { label: "Flip Volume", a: rA.flipVolume, b: rB.flipVolume, delta: rB.flipVolume - rA.flipVolume, suffix: "", lower: false },
    { label: "Flipper Sub", a: `${rA.currency}${rA.subPrice.flipper}`, b: `${rB.currency}${rB.subPrice.flipper}`, delta: rB.subPrice.flipper - rA.subPrice.flipper, suffix: "", lower: true },
    { label: "Pro Sub", a: `${rA.currency}${rA.subPrice.pro}`, b: `${rB.currency}${rB.subPrice.pro}`, delta: rB.subPrice.pro - rA.subPrice.pro, suffix: "", lower: true },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4 justify-center">
        <div className="flex items-center gap-2">
          <label htmlFor="regionA" className="text-xs text-neon font-display uppercase">Region A</label>
          <RegionSelect id="regionA" value={regionA} onChange={setRegionA} />
        </div>
        <span className="text-gray-500 font-display">vs</span>
        <div className="flex items-center gap-2">
          <label htmlFor="regionB" className="text-xs text-neon-purple font-display uppercase">Region B</label>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-gray-500 font-display text-xs uppercase">
              ...
              <th className="text-right py-2 pl-4">Delta</th>
            </tr>
          </thead>
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-gray-500 font-display text-xs uppercase">
              ...
              <th className="text-right py-2">Flipper</th>
              <th className="text-right py-2">Pro</th>
              <th className="text-right py-2">Avg Flip</th>
              <th className="text-right py-2 whitespace-nowrap">Sub % of Flip</th>
              <th className="py-2 pl-6 text-left min-w-[180px]">Affordability</th>
            </tr>
          </thead>
          <tbody className="font-mono">
            {regionKeys.map((key) => {
      ...
      <p className="text-xs text-gray-600 text-center italic">
        "Sub as % of flip" = Flipper subscription price / average flip check value.
        Lower is better — means one flip covers more months of service.
      </p>
    </div>
  );
}
          ...
          <div className="bg-surface-light border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-[10px] text-gray-500 uppercase font-display">Trust Score</div>
...
export default function Regions() {
  ...
  const [compareRegionA, setCompareRegionA] = useState("us");
  const [compareRegionB, setCompareRegionB] = useState("ca");
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 font-mono">
            Regional pricing, market analytics, and fraud prevention signals
          </p>
        </div>
