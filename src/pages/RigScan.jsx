...
const STATUS_COLORS = {
  ok: { bg: 'bg-neon-green/10', border: 'border-neon-green/40', dot: 'bg-neon-green', text: 'text-neon-green', label: 'OK' },
  weak: { bg: 'bg-neon-yellow/10', border: 'border-neon-yellow/40', dot: 'bg-neon-yellow', text: 'text-neon-yellow', label: 'Weak' },
  bottleneck: { bg: 'bg-neon-red/10', border: 'border-neon-red/40', dot: 'bg-neon-red', text: 'text-neon-red', label: 'Bottleneck' },
}
...
const TIER_STYLES = {
  Basic: { accent: 'neon-yellow', border: 'border-neon-yellow/30', glow: 'hover:shadow-[0_0_20px_rgba(255,230,0,0.15)]' },
  'Sweet Spot': { accent: 'neon-green', border: 'border-neon-green/30', glow: 'hover:shadow-[0_0_20px_rgba(57,255,20,0.15)]' },
  'New Build': { accent: 'neon-purple', border: 'border-neon-purple/30', glow: 'hover:shadow-[0_0_20px_rgba(191,90,242,0.15)]' },
}
    ...
    >
      ...
      {path.sellCurrent && (
        <div className="text-[10px] font-mono text-gray-500 mb-3 flex items-center gap-1">
          ...
          Sell current rig: ~${path.sellCurrent} &rarr; net ~${path.price - path.sellCurrent}
        </div>
      )}
    ...
    <div className="bg-surface-light border border-border rounded-xl p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-gray-500 text-xs font-mono mt-1">
            Estimated current rig resale value
          </p>
        </div>
      ...
      <div className="space-y-2 mb-5">
        ...
        <div className="flex items-center justify-between text-[10px] font-mono">
          <span className="text-gray-500">Low: <span className="text-neon-yellow">${low}</span></span>
          <span className="text-gray-400">Est: <span className="text-neon-green font-bold">${estimatedValue}</span></span>
          <span className="text-gray-500">High: <span className="text-neon-yellow">${high}</span></span>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-gray-500 font-mono text-sm">
            Diagnostics &amp; upgrade intelligence
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-surface-light border border-border rounded-xl p-6 flex flex-col items-center justify-center">
            ...
            <p className="text-gray-500 text-xs font-mono text-center mt-3">
              {rig.balanceIndex < 50
                ? 'Severe imbalance detected. Key components are limiting overall performance.'
                : rig.balanceIndex < 75
                  ? 'Moderate imbalance. Some components could use an upgrade.'
                  : 'Well-balanced rig. Minor tweaks could still help.'}
            </p>
          </div>
        </div>
