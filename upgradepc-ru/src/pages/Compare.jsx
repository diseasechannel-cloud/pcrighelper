...
const SPEC_ROWS = [
  { key: 'brand', label: 'Бренд', compare: false },
  { key: 'cores', label: 'Ядра', compare: true, higherBetter: true },
  { key: 'baseClock', label: 'Базовая частота', compare: true, higherBetter: true, parseNum: (v) => parseInt(v) },
  { key: 'boostClock', label: 'Буст частота', compare: true, higherBetter: true, parseNum: (v) => parseInt(v) },
  { key: 'vram', label: 'Видеопамять', compare: true, higherBetter: true, parseNum: (v) => parseInt(v) },
  { key: 'tdp', label: 'TDP', compare: true, higherBetter: false, unit: 'W' },
  { key: 'process', label: 'Техпроцесс', compare: true, higherBetter: false, parseNum: (v) => parseInt(v) },
  { key: 'price', label: 'Цена', compare: true, higherBetter: false, unit: '₽' },
]

const PERF_METRICS = [
  { key: 'gaming1080p', label: 'Игры 1080p' },
  { key: 'gaming1440p', label: 'Игры 1440p' },
  { key: 'gaming4k', label: 'Игры 4K' },
  { key: 'rayTracing', label: 'Трассировка лучей' },
...
export default function Compare() {
  ...
  const verdict = useMemo(() => {
    ...
    if (gamingDiff1080 !== 0) {
      ...
      parts.push(`${better}: лучше на ${pct}% в играх при 1080p`)
    }
    ...
    if (tdpDiff !== 0) {
      ...
      parts.push(`но ${higher} потребляет на +${Math.abs(tdpDiff)}Вт больше`)
    }
    ...
    if (vramA !== vramB) {
      ...
      parts.push(`${better}: +${Math.abs(vramA - vramB)}ГБ видеопамяти`)
    }
    ...
    if (effDiff !== 0) {
      ...
      parts.push(`${better}: лучшая эффективность`)
    }
    ...
    if (priceDiff !== 0) {
      ...
      parts.push(`${cheaper}: лучшая цена — ${Math.min(gpuA.price, gpuB.price).toLocaleString('ru-RU')}₽`)
    }
    ...
    if (rtDiff !== 0) {
      ...
      parts.push(`${better}: мощнее в трассировке лучей (+${Math.abs(rtDiff)}%)`)
    }
  ...
  if (!gpuA || !gpuB) {
    return <div className="text-gray-400 text-center py-12">Выберите две видеокарты для сравнения.</div>
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1 font-mono">Анализ компонентов бок о бок</p>
      </div>
      ...
      <div className="bg-surface-light border border-border rounded-xl overflow-hidden">
        <div className="grid grid-cols-3 bg-surface-lighter px-4 py-2 border-b border-border">
          <span className="text-xs text-gray-500 font-mono uppercase tracking-wider">Хар-ка</span>
