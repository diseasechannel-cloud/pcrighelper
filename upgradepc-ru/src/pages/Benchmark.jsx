...
const PRODUCTIVITY_TASKS = [
  { name: 'Видеомонтаж (DaVinci)', score: 72, percentile: 68, color: '#bf5af2' },
  { name: '3D Рендер (Blender)', score: 58, percentile: 55, color: '#00f0ff' },
  { name: 'Компиляция кода', score: 81, percentile: 79, color: '#39ff14' },
  { name: 'ИИ / ML нагрузки', score: 45, percentile: 42, color: '#ffe600' },
]

/* ─── Tabs ─── */
const TABS = ['Игры', 'Продуктивность', 'Температуры']
...
export default function Benchmark() {
  ...
  return (
    <div className="space-y-6">
      {/* ── Page Title ── */}
      <div>
        <h1 className="font-display text-2xl text-neon neon-text tracking-wide">Дашборд бенчмарков</h1>
        <p className="text-sm text-gray-500 mt-1 font-mono">Профилирование производительности и анализ температур</p>
      </div>
      ...
      <div className="bg-surface-light border border-border rounded-xl p-6 neon-glow">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          {/* Left — Hardware Name */}
          <div>
            <p className="text-xs text-gray-500 uppercase tracking-widest font-mono mb-1">Активный профиль железа</p>
          ...
          <div className="flex items-center gap-6">
            <ScoreRing value={42} label="1% мин. FPS" color="#ff3131" />
            <ScoreRing value={BENCHMARK_SCORES.gaming} label="Сред. FPS" color="#00f0ff" />
            <ScoreRing value={BENCHMARK_SCORES.thermals} label="Эффективность" color="#39ff14" />
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-light border border-border rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              <h3 className="font-display text-sm text-white tracking-wide">Тепловая карта FPS</h3>
              <span className="text-[10px] text-gray-500 font-mono">Ультра настройки · DLSS Выкл.</span>
            </div>
              ...
              <table className="w-full text-sm font-mono">
                <thead>
                  <tr className="text-gray-500 text-xs uppercase tracking-wider">
                    <th className="text-left px-5 py-3 font-medium">Игра</th>
                    <th className="text-center px-4 py-3 font-medium">Жанр</th>
          ...
          <div className="bg-surface-light border border-border rounded-xl p-5">
            <h3 className="font-display text-sm text-white tracking-wide mb-4">Распределение очков</h3>
            <p className="text-[11px] text-gray-500 font-mono mb-5">
              Ваш игровой результат vs. все протестированные ПК в базе UpgradePC
            </p>
      ...
      {/* ── Productivity Tab ── */}
      {activeTab === 'Продуктивность' && (
        <div className="bg-surface-light border border-border rounded-xl p-5 space-y-5">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-sm text-white tracking-wide">Бенчмарки продуктивности</h3>
            <span className="text-[10px] text-gray-500 font-mono">Общий: {BENCHMARK_SCORES.productivity}/100</span>
          </div>
      ...
      {/* ── Thermals Tab ── */}
      {activeTab === 'Температуры' && (
        <div className="space-y-6">
          {/* Thermal Chart */}
          <div className="bg-surface-light border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display text-sm text-white tracking-wide">Температура во времени</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'CPU Покой', value: `${THERMAL_DATA[0].cpuTemp}°C`, color: 'text-neon', sub: 'Фоновая нагрузка' },
              { label: 'GPU Покой', value: `${THERMAL_DATA[0].gpuTemp}°C`, color: 'text-neon-red', sub: 'Фоновая нагрузка' },
              { label: 'CPU Макс.', value: `${Math.max(...THERMAL_DATA.map(d => d.cpuTemp))}°C`, color: 'text-neon', sub: 'Стресс 30 мин' },
              { label: 'GPU Макс.', value: `${Math.max(...THERMAL_DATA.map(d => d.gpuTemp))}°C`, color: 'text-neon-red', sub: 'Стресс 30 мин' },
            ].map((stat) => (
          ...
          <div className="bg-neon-green/5 border border-neon-green/20 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-neon-green font-display font-bold tracking-wide">Температуры в безопасном диапазоне</p>
              <p className="text-xs text-gray-400 font-mono mt-1">
                Пиковые температуры остаются значительно ниже порога троттлинга 85°C.
                Ваше охлаждение справляется с длительными нагрузками.
              </p>
            </div>
          </div>
        </div>
